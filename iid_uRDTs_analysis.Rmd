...
plot.HRP2_prop<-iid_ldf %>% filter(test == "qPCR" | test == "RDT" | test == "uRDT") %>%
...
plot.qPCR_hist<-iid_ldf %>% filter(test == "HRP2" | test == "RDT" | test == "uRDT") %>%
  ggplot(aes(x = log10(pden_mean), color = result, fill = result)) +
  geom_histogram(position = "stack", bins = 60) +
  labs(y = "# of samples or iids",
       x = "Parasites/mL in log scale",
       color = "Result",
       fill = "Result") +
  facet_wrap(.~test, nrow = 3)+
  scale_color_manual(values = c("dodgerblue4", "firebrick3"))+
...
plot.qPCR_prop<-iid_ldf %>% filter(test == "HRP2" | test == "RDT" | test == "uRDT") %>%
...
lod.hrp2.plot <- lod.hrp2 %>% ggplot(aes(x = concentration, y = prob, color = test)) +
  ...
  geom_text(aes(x = lod.RDT.hrp2$lod$lod*1.1, y = 0.95), label = round(lod.RDT.hrp2$lod$lod, 2), color = "black", size = 3.2 )+
  geom_text(aes(x = lod.uRDT.hrp2$lod$lod*1.1, y = 0.95), label = round(lod.uRDT.hrp2$lod$lod, 2), color = "black", size = 3.2 )+
    theme_bw() +
  scale_color_manual(values = c("dodgerblue4", "firebrick4"))+
...
lod.qPCR.plot <- lod.qpcr %>% ggplot(aes(x = concentration, y = prob, color = test)) +
  ...
  geom_text(aes(x = lod.RDT.qpcr$lod$lod*1.1, y = 0.95), label = round(lod.RDT.qpcr$lod$lod, 2), color = "black", size = 3.2 )+
  geom_text(aes(x = lod.uRDT.qpcr$lod$lod*0.9, y = 0.95), label = round(lod.uRDT.qpcr$lod$lod, 2), color = "black", size = 3.2 )+
    theme_bw() +
  scale_color_manual(values = c("dodgerblue4", "firebrick4"))+
  scale_fill_manual(values = c("dodgerblue4", "firebrick4"))+
    labs(y = "Proportion of positive results",
...
plot1 <- plot_grid(plot_grid(plot.HRP2_hist,
                        plot.qPCR_hist,
                        ncol = 2),
                        plot_grid(plot.HRP2_prop$prop.plot,
                        plot.qPCR_prop$prop.plot,
                        ncol = 2),
                        plot_grid(lod.hrp2.plot,
                        lod.qPCR.plot,
                        ncol = 2), nrow = 3)
