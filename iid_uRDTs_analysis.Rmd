...
plot.HRP2_dot<-iid_ldf %>% filter(test == "HRP2" | test == "pan_LDH" |
                                    test == "pv_LDH" | test == "qPCR" |
                                    test == "RDT" | test == "uRDT") %>%
...
plot.HRP2_hist<-iid_ldf %>% filter(test == "HRP2" | test == "pan_LDH" |
                                    test == "pv_LDH" | test == "qPCR" |
                                    test == "RDT" | test == "uRDT") %>%
  ggplot(aes(x = log10(hrp2_pg_ml), color = result, fill = result)) +
  geom_histogram(alpha = 0.6, position = "stack", bins = 60) +
...
lod.RDT <- iid_ldf %>% filter(test == "RDT") %>% select (hrp2_pg_ml, result)%>%
...
lod.uRDT <- iid_ldf %>% filter(test == "uRDT") %>% select (hrp2_pg_ml, result)%>%
  fx_lod(concentration = "hrp2_pg_ml",
       result = "result",
       precision = 0.1,
       from = 0,
...
lod.RDT$glm.lod$test <- "RDT"
lod.uRDT$glm.lod$test <- "uRDT"

lod <- rbind(lod.RDT$glm.lod,lod.uRDT$glm.lod)

lod.plot <- lod %>% ggplot(aes(x = concentration, y = prob, color = test)) +
    geom_line()+
    geom_ribbon(aes(ymin = lower, ymax = upper, fill = test), alpha = 0.2, linetype = 3) +
    geom_vline(xintercept = c(lod.RDT$lod$lod, lod.RDT$lod$lower, lod.RDT$lod$upper), color = c("dodgerblue3", "dodgerblue1", "dodgerblue1"), size = 1, linetype = c(1,3,3)) +
    geom_vline(xintercept = c(lod.uRDT$lod$lod, lod.uRDT$lod$lower, lod.uRDT$lod$upper), color = c("firebrick3", "firebrick1", "firebrick1"), size = 1, linetype = c(1,3,3)) +
  geom_point(x = lod.RDT$lod$lod, y = 0.95, color = "black") +
  geom_point(x = lod.uRDT$lod$lod, y = 0.95, color = "black") +
  geom_text(aes(x = lod.RDT$lod$lod*1.1, y = 0.95), label = round(lod.RDT$lod$lod, 2), color = "black", size = 3.2 )+
  geom_text(aes(x = lod.uRDT$lod$lod*1.12, y = 0.95), label = round(lod.uRDT$lod$lod, 2), color = "black", size = 3.2 )+
  # geom_point(x = lod.RDT$lod$lower, y = 0.95, color = "gray40") +
  # geom_point(x = lod.uRDT$lod$lower, y = 0.95, color = "gray40") +
  # geom_text(aes(x = lod.RDT$lod$lower*0.92, y = 0.95), label = round(lod.RDT$lod$lower, 2), color = "gray40", size = 3.2) +
  # geom_text(aes(x = lod.uRDT$lod$lower*0.92, y = 0.95), label = round(lod.uRDT$lod$lower, 2), color = "gray40", size = 3.2) +
  # geom_point(x = lod.RDT$lod$upper, y = 0.95, color = "gray40") +
  # geom_point(x = lod.uRDT$lod$upper, y = 0.95, color = "gray40") +
  # geom_text(aes(x = lod.RDT$lod$upper*1.06, y = 0.95), label = round(lod.RDT$lod$upper, 2), color = "gray40", size = 3.2) +
  # geom_text(aes(x = lod.uRDT$lod$upper*1.06, y = 0.95), label = round(lod.uRDT$lod$upper, 2), color = "gray40", size = 3.2) +
    theme_bw() +
  scale_color_manual(values = c("dodgerblue4", "firebrick4"))+
  scale_fill_manual(values = c("dodgerblue4", "firebrick4"))+
    ...
    scale_x_continuous(expand = c(0, 0.02*max(lod$concentration)), breaks = seq(0, 20000, 2000)) +
    scale_y_continuous(expand = c(0, 0.02*max(lod$upper)), breaks = seq(0, 1, 0.1))
