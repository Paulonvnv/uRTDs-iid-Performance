...
plot.HRP2_hist<-ggplot(iid_ldf, aes(x = log10(hrp2_pg_ml), color = result, fill = result)) +
...
lod.analysis2 <- iid_ldf %>% filter(test == "uRDT") %>% select (hrp2_pg_ml, result)%>%
